...
#undef ALOGE
#define ALOGE(fmt, ...) ALOG(LOG_ERROR, LOG_TAG, "%s:%d: " fmt, basename(__FILE__), __LINE__, ##__VA_ARGS__)
#undef ALOGD
#define ALOGD(fmt, ...) do {} while(0)
//#define ALOGD(fmt, ...) ALOG(LOG_DEBUG, LOG_TAG, "%s:%d: " fmt, basename(__FILE__), __LINE__, ##__VA_ARGS__)
#undef ALOGI
#define ALOGI(fmt, ...) ALOG(LOG_INFO, LOG_TAG, "%s:%d: " fmt, basename(__FILE__), __LINE__, ##__VA_ARGS__)
...
#define FORWARD0(name) \
    static keymaster_error_t name(const keymaster1_device_t *_device) { \
...
static const TEEC_UUID UUID_TrustKernelKeymasterV1 = {
    0x2ea702fa, 0x17bc, 0x4752, { 0xb3, 0xad, 0xb2, 0x87, 0x1a, 0x77, 0x23, 0x47 } };

#define TEE_KEY_BLOB_SIZE 4096

#define CMD_GET_SUPPORT_ALGO_CNT 1
#define CMD_GET_SUPPORT_ALGO 2
#define CMD_GET_SUPPORT_BLK_MODE_CNT 3
#define CMD_GET_SUPPORT_BLK_MODE 4
#define CMD_GET_SUPPORT_PADDING_MODE_CNT 5
#define CMD_GET_SUPPORT_PADDING_MODE 6
#define CMD_GET_SUPPORT_DIGEST_CNT 7
#define CMD_GET_SUPPORT_DIGEST 8
#define CMD_GET_SUPPORT_IMPORT_FORMAT_CNT 9
#define CMD_GET_SUPPORT_IMPORT_FORMAT 10
#define CMD_GET_SUPPORT_EXPORT_FORMAT_CNT 11
#define CMD_GET_SUPPORT_EXPORT_FORMAT 12
...
struct TrustKernelKeymasterDevice {
    ...
    keymaster1_device_t *keymaster_device() { return &common; }
    hw_device_t *hw_device() { return &common.common; }

private:
    ~TrustKernelKeymasterDevice();
    keymaster1_device_t common;
    ...
    bool invoke(const char *file, uint32_t lineno, uint32_t cmdId, uint32_t va, uint32_t vb, void *inMem, uint32_t inMemSize, void *outMem, uint32_t *outMemSize, uint32_t *outA = NULL, uint32_t *outB = NULL) {
        ...
        op.params[2].tmpref.size = *outMemSize;
...
#define INVOKE(...) invoke(__FILE__, __LINE__, __VA_ARGS__)
     ...
     */
    FORWARD2(get_supported_algorithms,
             keymaster_algorithm_t**, algorithms,
             size_t*, algorithms_length) {
        uint32_t res;
        uint32_t cnt;

        if (!INVOKE(CMD_GET_SUPPORT_ALGO_CNT, 0, 0, NULL, 0, NULL, (uint32_t) 0, &res, &cnt))
        ...
        UniquePtr<keymaster_algorithm_t, Malloc_Free> pAlgo((keymaster_algorithm_t *) malloc(cnt * sizeof(keymaster_algorithm_t)));
        if (pAlgo.get() == NULL) {
            ALOGE("Out of memory");
            return KM_ERROR_MEMORY_ALLOCATION_FAILED;
        }
        if (!INVOKE(CMD_GET_SUPPORT_ALGO, 0, 0, NULL, 0, pAlgo.get(), cnt * sizeof(keymaster_algorithm_t), &res))
            return KM_ERROR_SECURE_HW_COMMUNICATION_FAILED;
        if (res != KM_ERROR_OK) {
            ALOGE("TEE return %d", res);
            return (keymaster_error_t) res;
        ...
        *algorithms = pAlgo.release();
        *algorithms_length = cnt;
        return KM_ERROR_OK;
    }
     ...
     */
    FORWARD4(get_supported_block_modes,
        ...
        if (!INVOKE(CMD_GET_SUPPORT_BLK_MODE_CNT, algorithm, purpose, NULL, 0, NULL, (uint32_t) 0, &res, &cnt))
            return KM_ERROR_SECURE_HW_COMMUNICATION_FAILED;
        if (res != KM_ERROR_OK) {
            ALOGE("TEE return %d", res);
            return (keymaster_error_t) res;
        ...
        UniquePtr<keymaster_block_mode_t, Malloc_Free> pModes((keymaster_block_mode_t *) malloc(cnt * sizeof(keymaster_block_mode_t)));
        if (pModes.get() == NULL) {
            ALOGE("Out of memory");
            return KM_ERROR_MEMORY_ALLOCATION_FAILED;
        }
        if (!INVOKE(CMD_GET_SUPPORT_BLK_MODE, algorithm, purpose, NULL, 0, pModes.get(), cnt * sizeof(keymaster_block_mode_t), &res))
            return KM_ERROR_SECURE_HW_COMMUNICATION_FAILED;
        if (res != KM_ERROR_OK) {
            ALOGE("TEE return %d", res);
            return (keymaster_error_t) res;
     ...
     * \param[out] modes_length Length of \p modes.
     */
    FORWARD4(get_supported_padding_modes,
             keymaster_algorithm_t, algorithm,
             keymaster_purpose_t, purpose,
        ...
        if (!INVOKE(CMD_GET_SUPPORT_PADDING_MODE_CNT, algorithm, purpose, NULL, 0, NULL, (uint32_t) 0, &res, &cnt))
            return KM_ERROR_SECURE_HW_COMMUNICATION_FAILED;
        if (res != KM_ERROR_OK) {
            ALOGE("TEE return %d", res);
            return (keymaster_error_t) res;
        ...
        UniquePtr<keymaster_padding_t, Malloc_Free> pModes((keymaster_padding_t *) malloc(cnt * sizeof(keymaster_padding_t)));
        if (pModes.get() == NULL) {
            ALOGE("Out of memory");
            return KM_ERROR_MEMORY_ALLOCATION_FAILED;
        }
        if (!INVOKE(CMD_GET_SUPPORT_PADDING_MODE, algorithm, purpose, NULL, 0, pModes.get(), cnt * sizeof(keymaster_padding_t), &res))
            return KM_ERROR_SECURE_HW_COMMUNICATION_FAILED;
        if (res != KM_ERROR_OK) {
            ALOGE("TEE return %d", res);
            return (keymaster_error_t) res;
     ...
     */
    FORWARD4(get_supported_digests,
             keymaster_algorithm_t, algorithm,
             keymaster_purpose_t, purpose,
        ...
        if (!INVOKE(CMD_GET_SUPPORT_DIGEST_CNT, algorithm, purpose, NULL, 0, NULL, (uint32_t) 0, &res, &cnt))
            return KM_ERROR_SECURE_HW_COMMUNICATION_FAILED;
        if (res != KM_ERROR_OK) {
            ALOGE("TEE return %d", res);
            return (keymaster_error_t) res;
        ...
        UniquePtr<keymaster_digest_t, Malloc_Free> pModes((keymaster_digest_t *) malloc(cnt * sizeof(keymaster_digest_t)));
        if (pModes.get() == NULL) {
            ALOGE("Out of memory");
            return KM_ERROR_MEMORY_ALLOCATION_FAILED;
        }
        if (!INVOKE(CMD_GET_SUPPORT_DIGEST, algorithm, purpose, NULL, 0, pModes.get(), cnt * sizeof(keymaster_digest_t), &res))
            return KM_ERROR_SECURE_HW_COMMUNICATION_FAILED;
        if (res != KM_ERROR_OK) {
            ALOGE("TEE return %d", res);
            return (keymaster_error_t) res;
     ...
     */
    FORWARD3(get_supported_import_formats,
             keymaster_algorithm_t, algorithm,
        ...
        if (!INVOKE(CMD_GET_SUPPORT_IMPORT_FORMAT_CNT, algorithm, 0, NULL, 0, NULL, (uint32_t) 0, &res, &cnt))
            return KM_ERROR_SECURE_HW_COMMUNICATION_FAILED;
        if (res != KM_ERROR_OK) {
            ALOGE("TEE return %d", res);
            return (keymaster_error_t) res;
        ...
        UniquePtr<keymaster_key_format_t, Malloc_Free> pModes((keymaster_key_format_t *) malloc(cnt * sizeof(keymaster_key_format_t)));
        if (pModes.get() == NULL) {
            ALOGE("Out of memory");
            return KM_ERROR_MEMORY_ALLOCATION_FAILED;
        }
        if (!INVOKE(CMD_GET_SUPPORT_IMPORT_FORMAT, algorithm, 0, NULL, 0, pModes.get(), cnt * sizeof(keymaster_key_format_t), &res))
            return KM_ERROR_SECURE_HW_COMMUNICATION_FAILED;
        if (res != KM_ERROR_OK) {
            ALOGE("TEE return %d", res);
            return (keymaster_error_t) res;
     ...
     * \param[out] formats_length Length of \p formats.
     */
    FORWARD3(get_supported_export_formats,
             keymaster_algorithm_t, algorithm,
             keymaster_key_format_t**, formats,
             size_t*, formats_length) {
        uint32_t res;
        ...
        if (!INVOKE(CMD_GET_SUPPORT_EXPORT_FORMAT_CNT, algorithm, 0, NULL, 0, NULL, (uint32_t) 0, &res, &cnt))
            return KM_ERROR_SECURE_HW_COMMUNICATION_FAILED;
        if (res != KM_ERROR_OK) {
            ALOGE("TEE return %d", res);
            return (keymaster_error_t) res;
        ...
        if (!INVOKE(CMD_GET_SUPPORT_EXPORT_FORMAT, algorithm, 0, NULL, 0, pModes.get(), cnt * sizeof(keymaster_key_format_t), &res))
            return KM_ERROR_SECURE_HW_COMMUNICATION_FAILED;
        if (res != KM_ERROR_OK) {
            ALOGE("TEE return %d", res);
            return (keymaster_error_t) res;
    ...
    static keymaster_key_characteristics_t *unmarshal_character(uint8_t *marshalled, uint32_t marshalled_size) {
        UniquePtr<keymaster_key_characteristics_t, Malloc_Free> pRes((keymaster_key_characteristics_t *) malloc(sizeof(keymaster_key_characteristics_t)));
        if (pRes.get() == NULL) {
            ALOGE("Out of memory");
            goto error;
        }
        pRes->hw_enforced.params = NULL;
        pRes->hw_enforced.length = 0;
        pRes->sw_enforced.params = NULL;
        pRes->sw_enforced.length = 0;

        pRes->hw_enforced = unmarshal_param_set(marshalled, marshalled_size);
        if (pRes->hw_enforced.params == NULL)
            goto error;
    ...
    error:
        if (pRes.get() != NULL) {
            if (pRes->hw_enforced.params != NULL)
                keymaster_free_param_set(&pRes->hw_enforced);
        }
        return NULL;
    }
    ...
    FORWARD5(export_key,
        ...
        uint32_t res;
        UniquePtr<keymaster_key_characteristics_t, Characteristics_Delete> characters;
        {
            keymaster_key_characteristics_t *key_params;
            if ((res = _get_key_characteristics(key_to_export, client_id, app_data, &key_params)) != KM_ERROR_OK) {
                ALOGE("Get key characteristics failed");
                return (keymaster_error_t) res;
            }
            characters.reset(key_params);
        }
    ...
    FORWARD5(finish,
        ...
        do {
            ...
            if (size != 0 && output == NULL)
                return KM_ERROR_OUTPUT_PARAMETER_NULL;
            output->data_length = size;
            pOutput.reset((uint8_t *) malloc(size));
            if (pOutput.get() == NULL) {
                ALOGE("Out of memory");
                return KM_ERROR_MEMORY_ALLOCATION_FAILED;
            }
            EXTRACT_BYTES_ERR(pOutput.get(), size, unmarshal_error);
            EXTRACT_PRIMITIVE_ERR(uint32_t, size, unmarshal_error);
...
TrustKernelKeymasterDevice::TrustKernelKeymasterDevice() {
    ...
    static_assert(offsetof(TrustKernelKeymasterDevice, common.common) == 0,
                  "common must be the first member of keymaster1_device");
#else
    assert(reinterpret_cast<keymaster1_device_t *>(this) == &device_);
    ...
    common.generate_keypair = NULL;
    common.import_keypair = NULL;
    common.get_keypair_public = NULL;
    common.delete_keypair = NULL;
    common.delete_all = NULL;
    common.sign_data = NULL;
    common.verify_data = NULL;

    common.get_supported_algorithms = get_supported_algorithms;
    common.get_supported_block_modes = get_supported_block_modes;
    common.get_supported_padding_modes = get_supported_padding_modes;
    common.get_supported_digests = get_supported_digests;
    common.get_supported_import_formats = get_supported_import_formats;
    common.get_supported_export_formats = get_supported_export_formats;
...
#define RETRY_TEEC_FUNC(name, ...) do { \
    ...
    RETRY_TEEC_FUNC(OpenSession, &context, &session,
            &UUID_TrustKernelKeymasterV1, TEEC_LOGIN_PUBLIC,
            NULL, NULL, &orig);
    if(error_ != TEEC_SUCCESS) {
